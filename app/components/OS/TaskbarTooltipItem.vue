...
const left = computed(() => {
  ...
  const value =
    props.containerBounds.left + props.containerBounds.width / 2 - tooltipBounds.value.width / 2

  return Math.max(Math.min(value, contentArea.value.width - tooltipBounds.value.width), 0)
})
...
<template>
  <div
    ...
    class="taskbar__tooltip"
    @mouseover="onMouseover"
    @mouseout="onMouseout"
  >
    <div ref="content" class="taskbar__tooltip__content">
      <OSTaskbarElementsProgramAllFrames :window-obs="windowObs" />
    </div>
  </div>
</template>
