 ...
 */
export function useGridCells(element: TemplateRef<HTMLElement | null>, preferredCell: Cell) {
  ...
  const calculateCells = () => {
    ...
    cellsInElement.x = Math.ceil(elementBounds.value.width / preferredCell.width)
    cellsInElement.y = Math.ceil(elementBounds.value.height / preferredCell.height)
  }

  // Подписывается на изменения размера элемента через ResizeObserver
  const subscribe = () => {
    if (!element.value) {
